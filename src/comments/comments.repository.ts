...
export class CommentsRepository extends Repository<Comment>{
    ...
    async getCommentById(lessonId :number,commentId:number):Promise<Comment>{
        ...
        .andWhere("comment.id = :commentid",{commentid:commentId}).getOne()

        return comment;
    }

    async updateComment(lessonId: number,commentId:number,user,updateData):Promise<any>{
       this.createQueryBuilder()
        .update(Comment).set({contents : updateData.contents,
                            rating: updateData.rating,
                            userId : user.id,
                            lessonId:lessonId
