...
export class SignupsService {
  ...
  async signup(id, updateData, user: User): Promise<any> {
    ...
    for (let i = 0; i < updateData.weekdays.length; i++) {
      const weekday = updateData.weekdays[i];
      const lessonTimeP = updateData.lessonTime[i];
      // for (const lessonTimeP in updateData.lessonTime) {
      console.log(updateData.weekdays[weekday], weekday);
      console.log(updateData);
