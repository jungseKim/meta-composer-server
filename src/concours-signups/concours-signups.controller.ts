...
export class ConcoursSignupsController {
    ...
    @ApiResponse({status: 200, description: '콩쿠르 참가 완료', type: ConcoursSignup })
    @ApiBody({ type: ConcoursSignup })
    //type 를 entity 로
    participate(@Body() updateData,@UserDecorator() user : User): Promise<ConcoursSignup> {

        return this
            .concoursSignupsService
            .participate(updateData,user);
    }
}
